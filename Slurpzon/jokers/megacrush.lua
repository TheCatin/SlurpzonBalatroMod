...
calculate = function(self, card, context)
    if context.before and context.cardarea == G.jokers then
        ...
        if card.ability.extra.megacrushuses > 0 then
            ...
            for _, j in ipairs(G.jokers.cards) do
                if j ~= card and j.config.center.blueprint_compat ~= false then
    ...
    if context.after and context.cardarea == G.jokers and not context.end_of_round then
        local had_target = card.ability.extra.target ~= nil
        card.ability.extra.target = nil
        -- Only spend a use if we actually had a valid target to fire
        if not had_target then return end
